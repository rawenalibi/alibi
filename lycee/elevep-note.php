                 ...
                 <div class="profile-content vl">
                  <img src="img/2.jpg" class="img-rounded" alt="Cinque Terre" width="680" height="150" style="margin-left:30px;"> 
                   <div class="page-header ">
                    <h3 style="color:#20B2AA" ><small class="pull-right"></small><span class="glyphicon glyphicon-pencil"></span>  Note</h3>
                  </div>                  
          
                  <?php 
                                        $s=$row['section'];
                                        $n=$row['niveau'];
                                    $stmt1 = $db->prepare("select * from note where section='$s' and niveau=$n");
                                    $stmt1->execute();       
                                    while($row1 = $stmt1->fetch() )
                                    { 
                                    ?>
                                        <div class="notice notice-success">
                                                <strong><?php echo $row1['matiere'] ?>  </strong>  
                                            
                                        <a  href="download-note.php?dow=<?php echo $row1['file'] ?>" >  telecharger  <p class="glyphicon glyphicon-download-alt"></p> </a> 
                                        </div>   
                                    <?php
                                    }
                ?>
